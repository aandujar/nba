<?php
//si hay algun campo en blanco redirijo a registro indicando error
if(empty($_POST["usuario"]) || empty($_POST["password"]) || empty($_FILES['avatar']['name'])){
    $_SESSION['errorRegistro']=true;
    header("Location: /registro");
}else{
    $_SESSION["extensionIncorrecta"] = null;
    $_SESSION["usuarioExiste"] = false;//variable de sesion para comprobar si usuario existe
    $_SESSION["errorBD"] = false;//variable de sesion por si hay errores en la bd
    $user = trim($_POST["usuario"]);
    $password = crypt($_POST["password"]);
    $avatar = basename($_FILES["avatar"]["name"]);

    if($_FILES["avatar"]["type"]=="image/png" || $_FILES["avatar"]["type"]=="image/jpg" || $_FILES["avatar"]["type"]=="image/jpeg" || $_FILES["avatar"]["type"]=="image/gif"){
